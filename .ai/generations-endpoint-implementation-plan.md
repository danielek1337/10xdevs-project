...
const createEntrySchema = z.object({
  ...
  task: z.string().trim().min(3, "Task must be at least 3 characters"),
...
5. **Entry-Tag Association:**
   ...
   if (tagIds.length > 0) {
     ...
     await supabase.from("entry_tags").insert(entryTagsData);
   }
   ```
  ...
  async resolveTagIds(tagNames: string[]): Promise<string[]> {
    ...
    if (missingTags.length > 0) {
      ...
      if (insertError?.code === "23505") {
        ...
        const { data: retryTags } = await this.supabase.from("tags").select("id, name").in("name", missingTags);
...
```typescript
import type { SupabaseClient } from "../db/supabase.client";
import type { CreateEntryDTO, EntryDTO, TagDTO, AntiSpamErrorResponseDTO } from "../types";
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const entry: EntryDTO = await entriesService.createEntry(user.id, validation.data);
